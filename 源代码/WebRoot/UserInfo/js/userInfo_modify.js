$(function () {
	...
	UE.delEditor('userInfo_skillContent_edit');
	var userInfo_skillContent_edit = UE.getEditor('userInfo_skillContent_edit'); //个人能力编辑器
	userInfo_skillContent_edit.addListener("ready", function () {
		 // editor准备好之后才可以使用 
		 ajaxModifyQuery();
	}); 
	...
	$.ajax({
		...
		success : function (userInfo, response, status) {
			...
			if (userInfo) { 
				...
				userInfo_skillContent_edit.setContent(userInfo.skillContent);
